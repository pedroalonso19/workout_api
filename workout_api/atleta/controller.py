...
async def post(
    ...
    if not categoria:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f'A categoria {categoria_name} não foi encontrada.')
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f'O centro de treinamento {centro_treinamento_nome} não foi encontrado.')
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Ocorreu um erro ao inserir os dados no banco.')
...
async def query(
    ...
    # Customiza resposta: retorna apenas nome, centro_treinamento, categoria
    result = [
        AtletaOut(
            nome=a.nome,
...
async def get(id: UUID4, db_session: DatabaseDependecy) -> AtletaOut:
    ...
    if not atleta:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Atleta não encontrado com id {id}')
...
async def get(id: UUID4, db_session: DatabaseDependecy) -> None:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel).filter_by(id=id))).scalars().first()
